...
/*   Updated: 2022/12/07 20:59:25 by moulmado         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	add_hsteps_till_wall(t_m_cords *horz, t_cub *cub, int ndx)
{
	...
	{
		if (map_has_wall_at(cub, horz->xinter, horz->yinter
				- cub->rays[ndx].isFacingUp) || door_check(cub, horz->xinter,
				horz->yinter - cub->rays[ndx].isFacingUp, ndx))
		{
			horz->door_hit = door_check(cub, horz->xinter,
					horz->yinter - cub->rays[ndx].isFacingUp, ndx);
...
static t_m_cords	get_horz_cords(t_cub *cub, int ndx)
{
	...
	horz.xinter = cub->player->x + (horz.yinter - cub->player->y)
		/ tan(cub->rays[ndx].angle);
...
static void	add_vsteps_till_wall(t_m_cords *vert, t_cub *cub, int ndx)
{
	vert->wallhitx = 1e18;
	vert->wallhity = 1e18;
	while (vert->xinter >= 0 && vert->xinter < cub->map_img->img_width
		&& vert->yinter >= 0 && vert->yinter < cub->map_img->img_height)
	{
		if (map_has_wall_at(cub, vert->xinter - cub->rays[ndx].isFacingLeft,
				vert->yinter) || door_check(cub, vert->xinter
				- cub->rays[ndx].isFacingLeft, vert->yinter, ndx))
		{
			vert->door_hit = door_check(cub, vert->xinter
					- cub->rays[ndx].isFacingLeft, vert->yinter, ndx);
...
static t_m_cords	get_vert_cords(t_cub *cub, int ndx)
{
	...
	vert.yinter = cub->player->y + (vert.xinter - cub->player->x)
		* tan(cub->rays[ndx].angle);
...
int	ray_cast(t_cub *cub, int ndx)
{
	...
	cub->rays[ndx].isFacingDown = cub->rays[ndx].angle
		> 0 && cub->rays[ndx].angle < M_PI;
	cub->rays[ndx].isFacingUp = !cub->rays[ndx].isFacingDown;
	cub->rays[ndx].isFacingLeft = cub->rays[ndx].angle
		> M_PI_2 && cub->rays[ndx].angle < 3 * M_PI_2;
	...
	if (horz.dst < vert.dst)
		return (cub->rays[ndx].dst = horz.dst,
			cub->rays[ndx].door_hit = horz.door_hit, HORZ);
	return (cub->rays[ndx].dst = vert.dst,
		cub->rays[ndx].door_hit = vert.door_hit, VERT);
}
