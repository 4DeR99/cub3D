...
/*   Updated: 2022/11/15 14:30:37 by moulmado         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void add_hsteps_till_wall(t_m_cords *horz, t_cub *cub, int ndx)
{
	horz->wallhitx = 1e18;
	horz->wallhity = 1e18;
	while (horz->xinter >= 0 && horz->xinter <= cub->win_width && horz->yinter >= 0 && horz->yinter <= cub->win_heigh)
	{
...
static void add_vsteps_till_wall(t_m_cords *vert, t_cub *cub, int ndx)
{
	vert->wallhitx = 1e18;
	vert->wallhity = 1e18;
	while (vert->xinter >= 0 && vert->xinter <= cub->win_width && vert->yinter >= 0 && vert->yinter <= cub->win_heigh)
	{
...
int ray_cast(t_cub *cub, int ndx)
{
	...
	if (horz.dst < vert.dst)
		return (cub->rays[ndx].dst = horz.dst,
				draw_line(cub, horz.wallhitx, horz.wallhity), 1);
	return (cub->rays[ndx].dst = vert.dst,
			draw_line(cub, vert.wallhitx, vert.wallhity), 2);
}
