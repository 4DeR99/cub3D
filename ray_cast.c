...
/*   Updated: 2022/11/25 10:51:28 by moulmado         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void add_hsteps_till_wall(t_m_cords *horz, t_cub *cub, int ndx)
{
	horz->wallhitx = 1e18;
	horz->wallhity = 1e18;
	while (horz->xinter >= 0 && horz->xinter <= cub->win_width &&
		horz->yinter >= 0 && horz->yinter <= cub->win_height)
	{
...
static t_m_cords get_horz_cords(t_cub *cub, int ndx)
{
	...
	horz.xinter = cub->player->x
			+ (horz.yinter - cub->player->y) / tan(cub->rays[ndx].angle);
...
static void add_vsteps_till_wall(t_m_cords *vert, t_cub *cub, int ndx)
{
	vert->wallhitx = 1e18;
	vert->wallhity = 1e18;
	while (vert->xinter >= 0 && vert->xinter <= cub->win_width
		&& vert->yinter >= 0 && vert->yinter <= cub->win_height)
	{
		if (map_has_wall_at(cub, vert->xinter
			- cub->rays[ndx].isFacingLeft, vert->yinter))
		{
...
static t_m_cords get_vert_cords(t_cub *cub, int ndx)
{
	...
	vert.yinter = cub->player->y
			+ (vert.xinter - cub->player->x) * tan(cub->rays[ndx].angle);
...
int ray_cast(t_cub *cub, int ndx)
{
	...
	if (horz.dst < vert.dst)
		return (cub->rays[ndx].dst = horz.dst, 1);
	return (cub->rays[ndx].dst = vert.dst, 2);
}
