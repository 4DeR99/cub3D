...
/*   Updated: 2022/12/07 14:43:19 by moulmado         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_ray(t_cub *cub, int ndx)
{
	...
	x2 = 12 * BLOCK_SIZE + cub->rays[ndx].dst*cos(cub->rays[ndx].angle);
	y2 = 7 * BLOCK_SIZE + cub->rays[ndx].dst*sin(cub->rays[ndx].angle);
	if (fabs(x2 - (12 * BLOCK_SIZE)) >= fabs(y2 - (7 * BLOCK_SIZE)))
		step = fabs(x2 - (12 * BLOCK_SIZE));
	else
		step = fabs(y2 - (7 * BLOCK_SIZE));
	xin = (float)(x2 - (12 * BLOCK_SIZE)) / step;
	yin = (float)(y2 - (7 * BLOCK_SIZE)) / step;
	x2 = 12 * BLOCK_SIZE;
	y2 = 7 * BLOCK_SIZE;
	k = 0;
	while (k++ < step && x2 < 24*BLOCK_SIZE && y2 < 14*BLOCK_SIZE)
	{
		x2 += xin;
		y2 += yin;
		my_mlx_pixel_put(cub->map_img, x2, y2, 0x00FFE8);
	}
}
