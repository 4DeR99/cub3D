...
/*   Updated: 2022/12/07 12:44:31 by moulmado         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void mem_alloc(t_cub *cub)
{
	...
	protect(cub->rend3D);
	cub->walls = (t_data *)calloc(5, sizeof(t_data));
	protect(cub->walls);
}
...
static void imgs_init(t_cub *cub)
{
	...
	cub->walls[DOOR].img = mlx_xpm_file_to_image(cub->mlx, "./assets/door.xpm",
			&cub->walls[DOOR].img_width, &cub->walls[DOOR].img_height);
	protect(cub->walls[DOOR].img);
}
...
static void	img_addr_init(t_cub *cub)
{
	...
	cub->walls[DOOR].colors = (int *)mlx_get_data_addr(cub->walls[DOOR].img,
			&cub->walls[DOOR].bits_per_pixel, &cub->walls[DOOR].line_length,
			&cub->walls[DOOR].endian);
	protect(cub->walls[DOOR].colors);
}
